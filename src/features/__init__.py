...
def create_loan_level_features(
    ...
    features_df = loan_creation_records.copy()

    # Add status at decision time
    status_at_decision = loan_status_at_decision[["loan_id", "status"]].rename(
        columns={"status": "status_at_decision_time"}
    )
    ...
    features_df = create_loan_characteristics(features_df)
    ...
    repayments_filtered = filter_repayments_by_decision_time(
    ...
    billing_features = create_billing_features(features_df, decision_time_days)
...
def create_cohort_level_features(features_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    Args:
        features_df: DataFrame with loan-level features
